...
async function fetchSuggestions(question: string, answer: string): Promise<string[]> {
  try {
    ...
    const prompt = `Based on this Q&A, suggest 3 short follow-up questions (max 8 words each). Return ONLY a JSON array of strings, nothing else, no explanation.
Q: ${question}
...
export function useChat(tenant: string, userName?: string) {
  ...
  const sendMessage = useCallback(async (question: string, sources: string[]) => {
    ...
    setLoading(true)
    try {
      const res = await query({ tenant, question, session_id: sessionId || undefined, sources: sources.length > 0 ? sources : undefined })
      if (res.session_id) { setSessionId(res.session_id); localStorage.setItem(SESSION_KEY(tenant), res.session_id) }
      setMessages((prev) => [...prev, { id: Date.now().toString() + '_bot', role: 'assistant', text: res.answer, sources: res.sources, confidence: res.confidence, ms: res.total_ms }])
      fetchSuggestions(question, res.answer).then(setSuggestions)
    } catch {
      setMessages((prev) => [...prev, { id: Date.now().toString() + '_err', role: 'assistant', text: 'Something went wrong. Please try again.' }])
    } finally { setLoading(false) }
  }, [loading, sessionId, tenant])

  const startNewConversation = useCallback(async () => {
    if (sessionId) { await clearSession(sessionId); localStorage.removeItem(SESSION_KEY(tenant)) }
    setSessionId(null); setSuggestions([]); setMessages([getWelcomeMessage(userName)])
  }, [sessionId, tenant, userName])

  return { messages, loading, restoring, bottomRef, suggestions, setSuggestions, sendMessage, startNewConversation, sessionId }
}
